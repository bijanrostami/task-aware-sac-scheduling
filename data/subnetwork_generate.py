...
'''
GW corresponds to gateway or AP
dv is device
First distributed APs on the grid according to the assumption (X,Y)
then
Then it distributes users among the APs, by randomly selecting distance
and angle of the user from the designated AP.

Finally it outputs the distance of devices from their APs, 
and the location of APs.

dist is the distance of devices from all APs
gwLoc is the location of APs.
'''
def create_layout(deploy_param):
    ...
    dist = cdist(gwLoc,dvLoc)  # Each row is for AP, Column for device
    return dist, gwLoc
...
'''
 returns a pathloss in terms of power coefficient (Linear).
 
 first computes the probability of LOS depending on distance.
 then it computes the pathloss given the formulas.

'''
def channel_pathLoss(deploy_param, dist):

    PrLoS = np.exp(dist * np.log(1 - deploy_param.clutDens) / deploy_param.clutSize)
    NLoS = PrLoS <= (1 - PrLoS) # No LOS
...
'''
Combination of all pathloss, shadowing and small-scale fading to form final power. all are in mW.
'''
def compute_power(deploy_param, dist, Loc, K):
    ...
    for k in range(K):
        h = (1 / np.sqrt(2)) * (deploy_param.rng_value.randn(N, N) + 1j * deploy_param.rng_value.randn(N, N))  #small-scale fading (multi-path)
        power[k, :, :] = PL * S_linear * np.power(np.abs(h), 2) #Combination of all pathloss, shadowing and small-scale fading, respectively.
    return power, PL
